...
> {
  ...
  static remapCompactToFull<SValue>(
    options: MenuOptionShort<SValue>
  ): MenuOption<SValue> {
  ...
  static remapFullToCompact<SValue>(
    options: MenuOption<SValue>
  ): MenuOptionShort<SValue> {
  ...
  constructor(public genericConfig: GenericConfig<TCtx, TState>) {
  ...
  get flatFilters(): MenuFilters<TValue> {
    return Array.isArray(this.genericConfig.filters[0])
      ? (this.genericConfig.filters as MenuFilters<TValue>[]).reduce(
          reduceArray
        )
  ...
  async sendMenu(ctx: TCtx) {
    ...
    const sendMessage = async () => {
      let sentMessage;
      if (
        this.genericConfig.customMessage &&
        this.genericConfig.customMessage.type === "image"
      ) {
        sentMessage = await ctx.replyWithPhoto(
          this.genericConfig.customMessage.content,
    ...
    if (oldMenu) {
      isReplacingMenu =
        oldMenu.genericConfig?.replaceable &&
        !oldMenu.deleted &&
        oldMenu.messageId !== this.messageId;
  ...
  protected toggleActiveButton(ctx: TCtx, activeButtons: TValue[]) {
    ...
    this.genericConfig.beforeChange?.(
      ctx as MenuContextUpdate<TCtx>,
      this.state
    );
  ...
  protected getButtonLabelInfo(ctx: TCtx, button: KeyboardButton<TValue>) {
    const isDefaultActiveButton =
      this.activeButtons.length === 0 && !!button.isDefault;
  ...
  private getMessage(ctx: TCtx) {
    const message =
      ctx.i18n?.t(this.genericConfig.message as string) ||
      this.genericConfig.message;
    return message + this.debugMessage;
  }

  private getSubmitMessage(ctx: TCtx) {
    return (
      ctx.i18n?.t(this.genericConfig.submitMessage) ||
      this.genericConfig.submitMessage ||
      "Submit"
    );
  }
  ...
  private onAction(ctx: MenuContextUpdate<TCtx>) {
    if (!ctx.state.callbackData) {
      throw new Error(
        "TelegrafMenu Error: You forgot to add middleware parser. Add following code to your bot:\n\n+++ bot.use(GenericMenu.middleware());\n"
      );
    }
  ...
  private getKeyboard(ctx: TCtx) {
    const filters: MenuFilters<TValue>[] = Array.isArray(
      this.genericConfig.filters[0]
    )
      ? (this.genericConfig.filters as MenuFilters<TValue>[])
      : ([this.genericConfig.filters] as MenuFilters<TValue>[]);
      ...
      return row.map((button) => {
        if (button.url) {
          return Markup.button.url(
            this.formatButtonLabel(ctx, button),
            button.url
          );
        }
        ...
        return Markup.button.callback(
          this.formatButtonLabel(ctx, button),
          JSON.stringify(shortButton)
        );
      });
    });

    if (
      this.genericConfig.onSubmit ||
      this.genericConfig.submitMessage ||
      this.genericConfig.onSubmitUpdater
    ) {
      const shortButton = GenericMenu.remapFullToCompact({
        action: this.genericConfig.action,
        value: "_local_submit",
